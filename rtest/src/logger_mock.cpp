  ...
  va_list * args)
{
  ...
  if (LoggerMock::instance_ != nullptr) {
    ...
    size_t len = vsnprintf(nullptr, 0, format, args_copy);   // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
    va_end(args_copy);   // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
    if (len == 0UL) {
      return;
    }
    msg.resize(len + 1);
    vsnprintf(&msg[0], len + 1, format, *args); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
